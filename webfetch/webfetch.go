// Package webfetch provides the web_fetch tool: it fetches one or more web
// pages, converts them to compact markdown, and (optionally) summarizes large
// pages with an LLM to keep token usage down.
//
// Security: every URL is untrusted. An SSRF guard (ssrf.go) blocks non-public
// addresses, embedded credentials, and non-http(s) schemes so a prompt-injected
// URL cannot reach internal services or cloud metadata.
package webfetch

import (
	"context"
	"fmt"
	"io"
	"log/slog"
	"net/http"
	"strings"
	"sync"
	"time"

	"github.com/sho0pi/agenttools/internal/safehttp"
	"github.com/sho0pi/agenttools/tool"
)

const (
	defaultMaxChars          = 8000
	defaultTotalMaxChars     = 24000
	defaultSummarizeMinChars = 5000
	defaultTimeout           = 15 * time.Second
	defaultUserAgent         = "agenttools-webfetch/1.0 (+https://github.com/sho0pi/agenttools)"
	maxURLs                  = 5
	maxBodyBytes             = 5 << 20 // 5 MiB cap on a single page download
)

// Config controls web_fetch behaviour. Pass the live values per call (via the
// cfgFn given to New) so config edits hot-reload.
type Config struct {
	MaxChars          int           // truncate each page to this many runes (0 → default)
	TotalMaxChars     int           // cap on combined output across all pages (0 → default)
	Summarize         bool          // summarize large pages via the Summarizer
	SummarizeMinChars int           // pages shorter than this skip the LLM (0 → default)
	Timeout           time.Duration // per-request timeout (0 → default)
	UserAgent         string        // outbound User-Agent (empty → default)
	BlockPrivate      bool          // SSRF guard on/off; SHOULD be true in production
	Logger            *slog.Logger  // optional; nil disables logging
}

func (c Config) withDefaults() Config {
	if c.MaxChars <= 0 {
		c.MaxChars = defaultMaxChars
	}
	if c.TotalMaxChars <= 0 {
		c.TotalMaxChars = defaultTotalMaxChars
	}
	if c.SummarizeMinChars <= 0 {
		c.SummarizeMinChars = defaultSummarizeMinChars
	}
	if c.Timeout <= 0 {
		c.Timeout = defaultTimeout
	}
	if c.UserAgent == "" {
		c.UserAgent = defaultUserAgent
	}
	return c
}

// Summarizer shrinks page content with an LLM. The web_fetch tool is
// decoupled from any specific LLM client through this interface; pass nil to
// disable summarization regardless of Config.Summarize.
type Summarizer interface {
	Summarize(ctx context.Context, content, instruction string) (string, error)
}

// Args are the web_fetch arguments.
type Args struct {
	URLs []string `json:"urls"`
}

// New returns the web_fetch tool, or an error if construction fails. cfgFn
// supplies live config per call (so config edits hot-reload); pass a
// constant-returning func for a fixed config, or nil for a safe default with the
// SSRF guard enabled. summarizer may be nil to disable LLM summarization.
func New(cfgFn func() Config, summarizer Summarizer) (tool.Tool, error) {
	if cfgFn == nil {
		cfgFn = func() Config { return Config{BlockPrivate: true} }
	}
	e := &extractor{cfgFn: cfgFn, summarizer: summarizer}
	return tool.NewTypedTool(
		"web_fetch",
		"Fetch the full content of one or more web pages by URL and return it as "+
			"markdown. Use after web_search when a snippet is not enough. Pass up to "+
			"5 URLs.",
		schema(),
		e.execute,
	), nil
}

func schema() *tool.Schema {
	return tool.Object(map[string]*tool.Property{
		"urls": {
			Type:        "array",
			Description: "Web page URLs to fetch (http/https), up to 5.",
			Items:       &tool.Property{Type: "string"},
		},
	}, "urls")
}

type extractor struct {
	cfgFn      func() Config
	summarizer Summarizer
}

// fetchResult holds one page's outcome, kept in URL order so concurrent fetches
// produce deterministic output.
type fetchResult struct {
	content string
	err     error
}

func (e *extractor) execute(ctx context.Context, args Args) (tool.Result, error) {
	cfg := e.cfgFn().withDefaults()

	urls := dedupeNonEmpty(args.URLs)
	if len(urls) == 0 {
		return tool.Result{}, fmt.Errorf("at least one url is required")
	}
	if len(urls) > maxURLs {
		urls = urls[:maxURLs]
	}

	client := newClient(cfg)

	// Fetch all pages concurrently. URL count is capped at maxURLs, so an
	// unbounded fan-out here is fine. Each result is written to its own slot,
	// so no mutex is needed; the slice index isolates writes.
	results := make([]fetchResult, len(urls))
	var wg sync.WaitGroup
	for i, raw := range urls {
		wg.Add(1)
		go func(i int, raw string) {
			defer wg.Done()
			content, err := e.fetchOne(ctx, client, cfg, raw)
			results[i] = fetchResult{content: content, err: err}
		}(i, raw)
	}
	wg.Wait()

	return e.assemble(cfg, urls, results), nil
}

// assemble joins per-URL results into a single markdown blob, enforcing the
// total output budget. Once the budget is exhausted, the offending section is
// truncated and remaining sections are summarized as skipped.
func (e *extractor) assemble(cfg Config, urls []string, results []fetchResult) tool.Result {
	var out strings.Builder
	data := make(map[string]any, len(urls))
	used := 0

	for i, raw := range urls {
		sep := ""
		if i > 0 {
			sep = "\n\n---\n\n"
		}

		r := results[i]
		if r.err != nil {
			section := fmt.Sprintf("## %s\n\nerror: %v", raw, r.err)
			out.WriteString(sep + section)
			data[raw] = map[string]any{"ok": false, "error": r.err.Error()}
			continue
		}

		remaining := cfg.TotalMaxChars - used
		if remaining <= 0 {
			data[raw] = map[string]any{"ok": false, "error": "skipped: output budget reached"}
			continue
		}
		section := truncateRunes(r.content, remaining)
		out.WriteString(sep + section)
		used += len([]rune(section))
		data[raw] = map[string]any{"ok": true, "chars": len([]rune(section))}
	}

	return tool.Result{Content: out.String(), Data: data}
}

func (e *extractor) fetchOne(ctx context.Context, client *http.Client, cfg Config, raw string) (string, error) {
	u, err := safehttp.ValidateURL(raw)
	if err != nil {
		return "", err
	}
	if cfg.BlockPrivate && safehttp.BlockedLiteralIP(u) {
		return "", fmt.Errorf("blocked non-public address %s", u.Hostname())
	}

	req, err := http.NewRequestWithContext(ctx, http.MethodGet, u.String(), nil)
	if err != nil {
		return "", err
	}
	req.Header.Set("User-Agent", cfg.UserAgent)
	req.Header.Set("Accept", "text/html,text/plain,*/*")

	resp, err := client.Do(req)
	if err != nil {
		return "", err
	}
	defer func() { _ = resp.Body.Close() }()

	if resp.StatusCode < 200 || resp.StatusCode >= 300 {
		return "", fmt.Errorf("http %d", resp.StatusCode)
	}

	bodyBytes, err := io.ReadAll(io.LimitReader(resp.Body, maxBodyBytes))
	if err != nil {
		return "", fmt.Errorf("read body: %w", err)
	}

	ctype := resp.Header.Get("Content-Type")
	title, body := renderBody(ctype, string(bodyBytes))

	body = strings.TrimSpace(body)
	if body == "" {
		return "", fmt.Errorf("no extractable text content")
	}

	body = e.shrink(ctx, cfg, u.String(), body)

	header := "## " + u.String()
	if title != "" {
		header = "## " + title + "\n" + u.String()
	}
	return header + "\n\n" + body, nil
}

// shrink summarizes the body when configured and large enough; otherwise it
// truncates to MaxChars. Summarization failures fall back to truncation so a
// flaky LLM never breaks extraction — the failure is logged when a Logger is set.
func (e *extractor) shrink(ctx context.Context, cfg Config, url, body string) string {
	if e.summarizer != nil && cfg.Summarize && len([]rune(body)) >= cfg.SummarizeMinChars {
		instruction := "Summarize the following web page into concise markdown, " +
			"preserving key facts, figures, names and quotes. Drop navigation and boilerplate. Source: " + url
		s, err := e.summarizer.Summarize(ctx, body, instruction)
		switch {
		case err != nil:
			if cfg.Logger != nil {
				cfg.Logger.WarnContext(ctx, "web_fetch: summarizer failed, falling back to truncation",
					slog.String("url", url), slog.Any("error", err))
			}
		case strings.TrimSpace(s) != "":
			return strings.TrimSpace(s)
		}
	}
	return truncateRunes(body, cfg.MaxChars)
}

// renderBody converts a response body to (title, text) based on content type.
func renderBody(contentType, body string) (title, text string) {
	if strings.Contains(strings.ToLower(contentType), "text/html") || looksLikeHTML(body) {
		return htmlToMarkdown(body)
	}
	// Plain text (or unknown): strip base64 image noise and return as-is.
	return "", stripBase64Images(body)
}

func looksLikeHTML(s string) bool {
	head := strings.ToLower(s)
	if len(head) > 512 {
		head = head[:512]
	}
	return strings.Contains(head, "<html") || strings.Contains(head, "<!doctype html") || strings.Contains(head, "<body")
}

func newClient(cfg Config) *http.Client {
	return safehttp.Client(safehttp.Config{Timeout: cfg.Timeout, BlockPrivate: cfg.BlockPrivate})
}

func dedupeNonEmpty(in []string) []string {
	seen := make(map[string]struct{}, len(in))
	out := make([]string, 0, len(in))
	for _, s := range in {
		s = strings.TrimSpace(s)
		if s == "" {
			continue
		}
		if _, ok := seen[s]; ok {
			continue
		}
		seen[s] = struct{}{}
		out = append(out, s)
	}
	return out
}

func truncateRunes(s string, max int) string {
	if max <= 0 {
		return s
	}
	r := []rune(s)
	if len(r) <= max {
		return s
	}
	return strings.TrimSpace(string(r[:max])) + "\n\n[... truncated]"
}
